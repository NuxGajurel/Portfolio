...
const page = () => {
  ...
  return (
   ...
   <div>
     ...
     <p className={`${dancingScript.className} text-2xl mt-3`}>Hi, I'm Nawaraj Gajurel (aka Nux), a passionate aspiring Full-Stack Developer from Nepal. This is the space where I showcase my work and the things I love to create.</p>
   </div>
<div className="flex items-center gap-4 mt-6">
  {links.map((item) => (
    <i key={item.name} className={`text-xl cursor-pointer hover:text-blue-500 hover:text-{item.name}`} aria-setsize={28}>
      {item.icons}
    </i>
  ))}
</div>
...
<p className='flex justify-end mt-3 text-gray-400 hover:text-black'> - Last Day of SEE (4/9/2026 Thursday)</p>
